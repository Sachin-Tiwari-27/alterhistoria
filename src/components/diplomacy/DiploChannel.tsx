...
export function DiploChannel() {
  ...
  const [searchResults, setSearchResults] = useState<typeof COUNTRIES[string][]>([])
  ...
  const send = async () => {
    ...
    activeChannel.forEach((id) => {
      addMessage(id, { from: 'player', fromName: player.customName ?? player.name, text, timestamp: Date.now() })
    })

    setLoading(true)
    const isGroup = activeChannel.length > 1
    const groupMemberNames = activeChannel.map((id) => nations[id]?.name ?? COUNTRIES[id]?.name ?? id)

    // Get AI response from each nation
    for (const targetId of activeChannel) {
      const nd = COUNTRIES[targetId]
      if (!nd) continue
      const system = buildDiplomacySystemPrompt(targetId, nd.name, nd.context, isGroup, groupMemberNames)
      ...
      try {
        ...
        addMessage(targetId, { from: targetId, fromName: nd.name, text: reply, timestamp: Date.now() })
      } catch {
        addMessage(targetId, { from: targetId, fromName: nd.name, text: 'No response received.', timestamp: Date.now() })
      }
    }
    setLoading(false)
  }
  ...
  if (!player) {
    return (
      <div className="flex-1 flex items-center justify-center p-6">
        <p className="text-xs text-muted-foreground italic text-center">Select a nation to open diplomatic channels.</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-b border-border flex-shrink-0 space-y-2">
        ...
        {activeChannel.length > 0 && (
          <div className="flex flex-wrap gap-1 items-center">
            <span className="font-cinzel text-[8px] tracking-widest text-muted-foreground uppercase">Channel:</span>
            {activeChannel.map((id) => {
              const nd = COUNTRIES[id]
              return (
                <span key={id} className="flex items-center gap-1 bg-muted border border-border rounded-sm px-1.5 py-0.5 text-[10px]">
                  {nd?.flag} {nd?.name ?? id}
                  <button onClick={() => removeFromChannel(id)} className="text-muted-foreground hover:text-destructive ml-0.5">
                    <X size={9} />
                  </button>
                </span>
              )
            })}
            <button onClick={clearChannel} className="text-[9px] text-muted-foreground hover:text-destructive ml-1">
              clear
            </button>
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3 min-h-0">
        {activeChannel.length === 0 && (
          <p className="text-xs text-muted-foreground italic text-center mt-4">
            Add nations above to open a diplomatic channel. Click a country on the map, or search above.
            You can create coalition groups with multiple nations.
          </p>
        )}
        ...
        {allMessages.map((m, i) => {
          ...
          return (
            <div key={i} className={`flex flex-col gap-0.5 ${isPlayer ? 'items-end' : 'items-start'}`}>
        ...
        {loading && (
          <div className="flex flex-col items-start gap-0.5">
            ...
            <div className="bg-muted border border-border rounded-sm px-3 py-3">
              <div className="thinking flex gap-1"><span /><span /><span /></div>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div className="flex gap-2 p-3 border-t border-border flex-shrink-0">
        <input
          ...
          placeholder={activeChannel.length ? 'Send diplomatic message…' : 'Add a nation first…'}
