...
const EVENT_STYLES: Record<WorldEvent['type'], { dot: string; border: string }> = {
    ...
    <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
      {events.map((e) => {
        ...
        return (
          <div
            key={e.id}
            className={`border-l-2 pl-3 py-1.5 ${style.border}`}
          >
            ...
            <p className="text-[11px] text-foreground leading-relaxed">{e.text}</p>
          </div>
        )
      })}
    </div>
